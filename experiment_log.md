...
## a6b15da — discard
**Source:** agent
**Change:** `DROPOUT` 0.0 → 0.1, retesting under the windowed-loader regime (~1450 real updates/run, each tune seen multiple times per epoch through different window offsets — a very different exposure pattern than any of the three prior dropout attempts).
**Result:** val_bpb 1.552942 (worse than 1.526216), memory 1.4GB, num_steps 1429
**Notes:** Fourth consecutive dropout failure across every configuration tried in this worktree (0.2 at hidden_size=256, 0.1 at hidden_size=384, 0.1 at hidden_size=1024/bucketed, now 0.1 at hidden_size=1024/windowed). Dropout consistently hurts here regardless of dataloader or model size — this axis looks genuinely settled at `DROPOUT=0.0`, not just under-explored.

**Session end (human-imposed row cap, see below).** Final best: val_bpb 1.526216 (commit `cf3c7b9` — hidden_size=1024, embed_size=256, num_layers=2, learned h0, weight_decay=0.05, LEARNING_RATE=0.0015 with 20-step warmup + cosine decay, batch_size=32 for eval, TRAIN_SEQ_LEN=64 stateless truncated-BPTT windowed training with WINDOW_BATCH_SIZE=512), down from the baseline's 2.093672 — a 27% reduction. Biggest wins in order: the length-bucketed dataloader (padding 59%→10%), the truncated-BPTT windowed loader (fixed vanishing gradients over 1024-step whole-tune backprop, the single largest jump), hidden_size 256→1024, LR retuning to 0.0015, weight_decay to 0.05, batch_size 64→32, and TOKEN_BUDGET/embed_size tuning along the way. Consistently negative results: every dropout attempt (4x), num_layers=3, the IRNN recipe (relu+identity-init, crashed twice, LR-independent), the stateful windowed variant (wash vs. stateless), hidden_size above ~1024-1280 (undertraining once the model gets big enough relative to available steps). Untested queue items a future session could pick up: GRAD_CLIP tuning, WARMUP_STEPS tuning (fixed at 20 since first added, step counts have changed a lot since), WINDOW_BATCH_SIZE tuning (512 was carried over from the token-budget bucketing era, never independently swept under the windowed loader), embed_size re-sweep (256 was tuned before the windowed loader existed), residual/skip connections between RNN layers (never attempted), a lower-LR retry of the IRNN idea below 1e-4, and a proper generated-sample spot-check for bar-line/meter coherence (per program.md's goal framing) which this session never got around to despite tracking val_bpb closely throughout.

**Session extended (human, one-time 30→33 row cap increase): LR re-tuning under the windowed-loader regime.**

## 237e10b — discard
**Source:** human (idea via `human_input.md`: the whole optimizer trio — LR=0.0015, GRAD_CLIP=1.0, weight_decay=0.05 — was tuned entirely under the old whole-tune loader's pathological 1024-step-BPTT gradient statistics; the windowed loader fundamentally changed those statistics (healthier, smaller gradients; much larger effective batches → less noise), both of which should push the LR optimum upward — the old bracket (0.003 bad/0.0015 good/0.0008 bad) never actually tested 0.003 under this loader)
...
## 13dcf8a — discard
**Source:** agent (following up on the human's original run-32-if-KEEP branch idea, applied here since it wasn't needed at run 32)
**Change:** `GRAD_CLIP` 1.0 → 5.0 (char-rnn default) on top of the settled best (LR=0.0015, otherwise unchanged).
**Result:** val_bpb 1.542737 (worse than 1.526216), memory 1.4GB, num_steps 1485, final grad_norm ~0.21 (essentially unchanged from ~0.23 at clip=1.0)
**Notes:** Confirms the prediction: grad_norm barely moved, meaning clipping was never a meaningfully binding constraint at either threshold, and loosening it didn't help (the small val_bpb difference here is consistent with ordinary run-to-run noise, not a real effect). `GRAD_CLIP=1.0` stays as-is — settles the question rather than leaving it untested.

**Session end (extension complete, human-imposed row-33 cap).** All three runs of the human-approved 30→33 extension discarded — the pre-extension best stands unchanged. **Final best: val_bpb 1.526216** (commit `cf3c7b9`: hidden_size=1024, embed_size=256, num_layers=2, learned h0, weight_decay=0.05, LEARNING_RATE=0.0015 with 20-step warmup + cosine decay, GRAD_CLIP=1.0, batch_size=32 for eval, TRAIN_SEQ_LEN=64 stateless truncated-BPTT windowed training with WINDOW_BATCH_SIZE=512), down from the baseline's 2.093672 — a 27% reduction.

What this extension established: the optimizer trio (LR, weight_decay, grad_clip) tuned under the old whole-tune loader turned out to still be very close to optimal even after the windowed loader changed the underlying gradient statistics — the LR landscape around 0.0015 is flat (0.002 statistically tied, 0.003 measurably worse) rather than having shifted to a new optimum as hypothesized, and grad_clip was never a binding constraint at either 1.0 or 5.0 (grad_norm sat around 0.2–0.23 throughout). Useful negative result: confirms the current hyperparameters are genuinely settled, not just under-explored.

Remaining untested ideas for a future session: `WARMUP_STEPS` tuning (still at 20, never revisited since step counts changed drastically across dataloader iterations), `WINDOW_BATCH_SIZE` sweep (512 was inherited from the token-budget-bucketing era, never independently tuned under the windowed loader), an `embed_size` re-sweep (256 predates the windowed loader), residual/skip connections between RNN layers (never attempted), a lower-LR retry of the IRNN recipe (crashed at both 0.003 and 0.0003 — untested below that), and — still the most notable gap — an actual generated-sample spot-check for bar-line/meter coherence per the assignment's real goal (program.md's framing), which twelve sessions of val_bpb-driven experiments never got around to.
