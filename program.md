...
**Worktree scope**: swap `nn.RNN` for `nn.LSTM` in `CharRNN` as your first experiment (baseline run is still the unmodified `nn.RNN` per the setup protocol below). After that, **stay on `nn.LSTM` — do NOT swap to `nn.GRU`, bidirectional, plain `nn.RNN`, or any other cell type.** That is what the parallel `gru-improve` / `bidirectional-improve` / `architecture-search` worktrees are for; one fixed cell type per worktree is exactly what makes the cross-worktree ablation comparison valid. Your mandate to originate your own architectural ideas (see "Driving your own research") operates *within the LSTM family* — gating tweaks, layer structure, dataloader, initialization, optimizer, LR schedule, hidden_size, embed_size are all fair game; the LSTM cell-type identity of this worktree is the one thing that is fixed.

**Sequence-length stance (this is the point of this worktree, not a side note):** `baseline-improve` solved the vanilla-RNN vanishing-gradient problem by *avoiding* long backprop — it cut training to `TRAIN_SEQ_LEN=64` truncated-BPTT windows. That is a fair win for a vanilla RNN, but it dodges the question this worktree exists to answer: **does an LSTM, with its cell-state memory, actually beat the RNN when long-range context is genuinely in the training signal?** Inheriting `baseline-improve`'s `TRAIN_SEQ_LEN=64` windowed loader verbatim would make the LSTM just a bigger-parameter RNN on short windows and would prove nothing about the assignment's goal (long-range bar-line/meter coherence). So: do NOT copy the `TRAIN_SEQ_LEN=64` windowed loader as your default. Start the LSTM on **longer sequences** (try `TRAIN_SEQ_LEN` 128 / 256 / 512, or fall back to whole-tune `make_dataloader` if a windowed loader at those lengths proves hard) and accept that this means **fewer steps per 300s run** — that's the tradeoff that makes the comparison meaningful. Do carry over `baseline-improve`'s non-cell findings that aren't sequence-length-specific: learned initial hidden state, `WEIGHT_DECAY=0.05`, `LEARNING_RATE=0.0015` with 20-step warmup + cosine decay, `EMBED_SIZE=256`, `BATCH_SIZE=32` for eval. Expect `tokens_M` per run to be well below baseline-improve's `~47M` windowed figure — the `keep-prov` rule above exists for exactly this case, and the right re-test lever here is `TRAIN_SEQ_LEN` (sequence length), not `HIDDEN_SIZE`.

## Transferable learnings from baseline-improve

`autoresearch/baseline-improve` ran 33 experiments on the plain `nn.RNN` and drove val_bpb from 2.093672 → **1.526216** (its full `results.tsv`/`experiment_log.md` are the source of truth — read them for detail). You do NOT need to rediscover any of this. After your run-1 baseline and run-2 raw LSTM swap, use these to skip dead-ends and jump to a strong config instead of re-deriving it hyperparameter by hyperparameter.

**Adopt as your starting stack (architecture-agnostic wins):**
- **Windowed truncated-BPTT dataloader** — the single biggest win there (val_bpb 1.61→1.53). A `train.py`-only loader (`prepare.py` stays untouched) that cuts each tune into fixed-length windows instead of feeding whole padded tunes. It kills padding waste (~59%→~10%) and multiplies gradient updates in the fixed budget. See baseline's `cf3c7b9` entry for the implementation shape. **But re-tune the window length — 64 was RNN-specific (see below).**
- **Time-based LR warmup + cosine decay** (scheduled on `total_training_time / TIME_BUDGET`, not step count — step counts vary wildly here). Clear win, transfers.
- **Learned initial hidden state** (`h0` as `nn.Parameter`). Small but real and cheap. For an LSTM this is a learned `(h0, c0)` pair — both the hidden AND the cell state.
- **weight_decay ≈ 0.05** (AdamW decoupled decay) as the regularizer of choice — re-confirm the value once, but expect decay-not-dropout to be right here.
- **embed_size 256 > 128** — input-representation win, orthogonal to the recurrence, should transfer directly.
- **Methodology**: measure padding waste before reworking the loader; always fill `num_steps`/`tokens_M`/`num_params_M` — the fixed-time budget makes throughput the hidden variable behind most apparent "wins."

**Skip entirely (settled negatives, or RNN-specific):**
- **Dropout** — failed 4× across every size/loader combo on the RNN. LSTMs overfit a bit more readily, so you get *one* check at low dropout (~0.1) once you have a strong config; if it hurts like it did for RNN, drop it for good, don't sweep it.
- **The IRNN recipe** (`nonlinearity='relu'` + identity-init `weight_hh`) — crashed twice (forward-pass activation explosion, LR-independent). It's a vanilla-RNN vanishing-gradient hack; the LSTM's gates solve that structurally, so it is **irrelevant to you — do not try it.**
- **`num_layers=3`** — hurt (extra per-step compute → fewer updates in the fixed budget). Even more true for LSTM (4 gates ≈ 4× the per-layer cost). Stay at 2 unless you have throughput to spare.

**Re-tune fresh — an LSTM is not an RNN, these WILL differ:**
- **`hidden_size`** — baseline settled on 1024, but an LSTM cell has ~4× the parameters and compute per hidden unit (4 gates vs 1). At the same throughput ceiling your sweet spot lands **lower** — start around 384–512 and sweep, do NOT blindly copy 1024. This is exactly why `num_params_M` is now a ledger column: compare cell types like-for-like on params, not on hidden_size.
- **Window length** — 64 was tuned to the vanilla RNN's ~10–50-step effective memory (32 and 128 both bracketed worse). An LSTM's whole point is *longer* effective memory, so it may prefer **longer windows (128, 256+)** — sweep this deliberately. It's one of the few places the LSTM can genuinely beat the RNN on the assignment's real goal (long-range meter/bar structure).
- **Stateful vs stateless windowing** — for the RNN, carrying hidden state across a tune's windows was a wash (its memory is too short to use the extra context). For an LSTM this is precisely the capability that should matter — **re-test the stateful variant**; it may be a real win here where it wasn't for RNN.
- **LR value** — 0.0015 was the RNN optimum; the gates change gradient scale, so re-bracket it for LSTM.

**For the ablation write-up (bonus point):** the cleanest architecture comparison is baseline-improve's best config with *only* the cell swapped — same windowed loader, same hidden_size, same everything, `nn.RNN`→`nn.LSTM`. Run that exact comparison at least once and record the pair (RNN 1.526216 vs LSTM X) as your headline ablation number, kept separate from your own tuning trajectory.
