...
**Worktree scope**: swap `nn.RNN` for `nn.GRU` in `CharRNN` as your first experiment (baseline run is still the unmodified `nn.RNN` per the setup protocol below), then push everything else — hidden_size, num_layers, dropout, optimizer, LR schedule.
